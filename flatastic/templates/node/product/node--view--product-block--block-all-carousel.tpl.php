...
if (isset($node->field_product_attributes)):
    if(isset($node->field_product_attributes['und'])){
        foreach ($node->field_product_attributes['und'] as $att) {
            $str_att .= ' filter_class_' . strtolower($att['taxonomy_term']->name);
        }
    }
endif;
...
?>
<figure class="r_corners photoframe shadow relative animate_ftb long tr_all_hover <?php print $str_att; ?>">
    <!--product preview-->
    <a href="<?php echo $node_url ?>" class="d_block relative pp_wrap">
        <!--hot product-->
        <?php if(theme_get_setting('demo') == 'interior'):?>
            <?php if (strpos($str_att,'filter_class_hit')): ?>
                <span class="hot_interior"><?php print t('HOT');?></span>
            <?php endif; ?>
            <?php if (strpos($str_att,'filter_class_specials')): ?>
    ...
    <figcaption>
        ...
        <div class="clearfix relative">
            ...
            <?php if (module_exists('flag')): ?>
                <div class="p_right"><?php print flag_create_link('shop', $node->nid); ?></div>
            <?php endif; ?>
        </div>
    </figcaption>
</figure>